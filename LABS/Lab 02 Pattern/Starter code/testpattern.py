...
class TestPattern(unittest.TestCase):
    def testfindMatch(self):
        pat = Pattern('abc')
        text = 'abbabcabc'
        self.assertEqual(pat._findMatch(text), 3)

    def testfindmatchwildcard(self):
        pat = Pattern('a*c', '*')
        text = 'aaaabbaac'
        self.assertEqual(pat._findMatch(text), 6)
    ...
    def testmisfindmatch(self):
        pat = Pattern('abc')
        text = 'abababab'
        self.assertEqual(pat._findMatch(text), -1)
    ...
    def testlongexample(self):
        ...
        self.assertEqual(pat._findMatch(text), 1000)

    def testfindmatchcasesensitive(self):
        p = Pattern("abc")
        text = 'ABCababcabababc'
        self.assertEqual(pat._findMatch(text), 0)
        p._set_case_sensitive(True)
        self.assertEqual(pat._findMatch(text), 5)
    ...
    def testfindMatches(self):
        pat = Pattern('iss')
        text = 'mississippi'
        self.assertEqual(pat._findMatches(text), [1, 4])

    def testfindmatcheswildcard(self):
        pat = Pattern('i**', '*')
        text = 'mississippi'
        self.assertEqual(pat._findMatches(text), [1, 4, 7])

    def testfindmatchesotherwildcard(self):
        pat = Pattern('si?', '?')
        text = 'mississippi'
        self.assertEqual(pat._findMatches(text), [3, 6])

    def testmisfindmatches(self):
        pat = Pattern('isi')
        text = 'mississippi'
        self.assertEqual(pat._findMatches(text), [])
    ...
    def testfindmatchescasesensitive(self):
        p = Pattern("abc")
        text = 'ABCababcabababc'
        self.assertEqual(pat._findMatches(text), [0, 5, 12])
        p._set_case_sensitive(True)
        self.assertEqual(pat._findMatches(text), [5, 12])
    ...
    def testtimeonepatternanyindex(self):
        ...
        starttime = time.time()
        self.assertEqual(testedPattern._findMatches(reallyLong), [randPos])
